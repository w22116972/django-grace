...
def setup_nginx():
    upload_template(
        filename='config/nginx.conf', #local
        destination='/etc/nginx/sites-enabled/test.conf', #remote
        context={ # overwrite variable in templates
            'server_name': 'localhost',
            'static_dir': os.path.join(DJANGO_DIR, 'static'),
            'media_dir': os.path.join(DJANGO_DIR, 'media'),
        },
        use_sudo=True, # /etc/ 只能sudo
        use_jinja=True 
    )
